...
*
* $Id: class.tx_devlog_remote.php 40423 2010-11-18 17:16:19Z fabien_u $
***************************************************************/
...
class tx_devlog_remote {
	...
	public function __construct() {
		global $LANG;
		$this->parameters = array_merge(\TYPO3\CMS\Core\Utility\GeneralUtility::_GET(), \TYPO3\CMS\Core\Utility\GeneralUtility::_POST());
...
//		\TYPO3\CMS\Core\Utility\GeneralUtility::debug($request, '$datasource');
...
//		\TYPO3\CMS\Core\Utility\GeneralUtility::debug($datasource, '$datasource');
		// For JsonReader
...
//    },
    ...
    function formatPid($uid) {
		...
		$elementIcon = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIconForRecord('pages', array());
	...
	protected function formatCruser($uid = 0) {
		global $TCA;
		$row = \TYPO3\CMS\Backend\Utility\BackendUtility::getRecord('be_users', $uid);
		$elementTitle = \TYPO3\CMS\Backend\Utility\BackendUtility::getRecordTitle('be_users', $row, 1);
		$spriteName = $TCA['be_users']['ctrl']['typeicon_classes'][$row['admin']];
		$elementIcon = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon($spriteName);
	...
	public function getDataVar() {
		...
		if ($uid) {
			...
			if (isset($records[0]['data_var']) && $records[0]['data_var'] != '') {
				...
				$result = \TYPO3\CMS\Core\Utility\DebugUtility::viewArray(@unserialize($dataVar));
			}
		}
		echo $result;
	}
