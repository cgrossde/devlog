...
*
*  $Id: index.php 40424 2010-11-18 17:19:50Z fabien_u $
***************************************************************/
...
//		$this->selectLog();
		...
		$this->javascriptPath = \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extRelPath('devlog') . 'Resources/Public/javascripts/';
...
//			$markers['###MENUBAR###'] = $this->renderMenuBar();
			...
			$backendTemplateFile = \TYPO3\CMS\Core\Utility\GeneralUtility::getFileAbsFileName('EXT:devlog/Resources/Private/Templates/index.html');
			$this->content .= \TYPO3\CMS\Core\Html\HtmlParser::substituteMarkerArray(file_get_contents($backendTemplateFile), $markers);
		}
		else {
				// If no access
	...
	protected function loadStylesheets() {
		$path = \TYPO3\CMS\Core\Utility\ExtensionManagementUtility::extRelPath('devlog');
...
EOF;
		...
		$imageExpand = \TYPO3\CMS\Backend\Utility\IconUtility::skinImg($BACK_PATH, 'gfx/plusbullet_list.gif','width="18" height="12"');
		$imageCollapse = \TYPO3\CMS\Backend\Utility\IconUtility::skinImg($BACK_PATH, 'gfx/minusbullet_list.gif','width="18" height="12"');
...
EOF;
	...
	public function getLogPeriod() {
		...
		if ($startDate > 0 && $endDate > 0) {
			...
			if ($startDate != $endDate) {
				$content = $LANG->getLL('log_period').': '.\TYPO3\CMS\Backend\Utility\BackendUtility::dateTimeAge($startDate).' - '.\TYPO3\CMS\Backend\Utility\BackendUtility::dateTimeAge($endDate);
			}
			else {
				$content = $LANG->getLL('log_period') . ': '.\TYPO3\CMS\Backend\Utility\BackendUtility::dateTimeAge($startDate);
			}
		}
	...
	public function getSeverityList() {
		...
		while ($row = $GLOBALS['TYPO3_DB']->sql_fetch_assoc($dbres)) {
			...
			$icon = \TYPO3\CMS\Backend\Utility\IconUtility::getSpriteIcon('extensions-devlog-' . $severity);
...
EOF;
	...
	private function processParameters() {
		$message = '';
		$parameters = \TYPO3\CMS\Core\Utility\GeneralUtility::_GP('tx_devlog');
...
EOF;
	...
	function openNewView() {
		...
		$content = '<a id="openview" href="#" onclick="' . htmlspecialchars($onClick).'">' .
					'<img' . \TYPO3\CMS\Backend\Utility\IconUtility::skinImg($BACK_PATH,'gfx/open_in_new_window.gif', 'width="19" height="14"') . ' title="' . $GLOBALS['LANG']->sL('LLL:EXT:lang/locallang_core.php:labels.openInNewWindow', 1) . '" class="absmiddle" alt="" />' .
					'</a>';
		return $content;
	}
}
