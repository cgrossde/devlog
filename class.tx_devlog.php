 ...
 *
 *  $Id: class.tx_devlog.php 40908 2010-12-06 21:30:39Z francois $
 */
class tx_devlog {
	...
	public function devLog($logArr) {
		...
		if (!isset($GLOBALS['TYPO3_DB']) || !is_object($GLOBALS['TYPO3_DB']) || !$GLOBALS['TYPO3_DB']->getDatabaseHandle()) {
			return;
		}
...
//		$logEntry['crdate'] = $GLOBALS['TYPO3_CONF_VARS']['EXTCONF'][$this->extKey]['tstamp'];
		...
		$logEntry['ip'] = \TYPO3\CMS\Core\Utility\GeneralUtility::getIndpEnv('REMOTE_ADDR');

			// Clean up the message
		$logEntry['message'] = \TYPO3\CMS\Core\Utility\GeneralUtility::removeXSS($logArr['msg']);
		...
		foreach ($GLOBALS['TYPO3_CONF_VARS']['EXTCONF'][$this->extKey]['writers'] as $class) {
			...
			$writerObject = \TYPO3\CMS\Core\Utility\GeneralUtility::makeInstance($class);
